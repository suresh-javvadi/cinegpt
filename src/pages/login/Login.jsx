...
const Login = () => {
  ...
  // Set when a hero CTA is clicked while signed out — where to send the user after auth
  const [pendingRoute, setPendingRoute] = useState(null);
  const authColumnRef = useRef();
    ...
    if (mode === "signin") {
      signInWithEmailAndPassword(
        ...
        .then(() => {
          ...
          setIsSuccess(true);
          navigate(pendingRoute?.path ?? "/browse", { replace: true });
        })
        .catch((error) => {
          setIsLoading(false);
  ...
  // Hero CTA gate: signed-in users go straight through, everyone else is walked to the form
  const handleRequireAuth = (path, label) => {
    if (user) {
      navigate(path);
      return;
    }
    ...
    authColumnRef.current?.scrollIntoView({
      behavior: "smooth",
      block: "center",
    });
    setTimeout(() => email.current?.focus({ preventScroll: true }), 400);
  };
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden">
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center px-4 py-24 sm:py-20">
        <div className="w-full max-w-6xl flex flex-col lg:flex-row items-center justify-center lg:justify-between gap-14 lg:gap-16">
          {/* Hero — marketing column */}
          <LandingHero onRequireAuth={handleRequireAuth} />

          {/* Auth column */}
          <div ref={authColumnRef} className="w-full max-w-md lg:shrink-0">
            {/* Headline — outside card */}
            <div className="text-center mb-8">
              <h1 className="text-4xl sm:text-5xl lg:text-3xl font-black text-white tracking-tight leading-tight">
                {mode === "forgot" ? (
                  <>
                    <span
                      style={{
                  ...
                  <>
                    ...
                    <br />
                    <span className="text-red-500">CineGPT</span>
                  </>
                )}
              </h1>
            ...
            >
                  ...
                  {forgotStatus === "sent" ? (
                    ...
                    <div className="text-center py-4">
                      ...
                      <p className="text-gray-500 text-xs mb-7">
                        Didn't receive it? Double-check the email or look in
                        your spam folder.
                      </p>
                      <button
                        type="button"
              ...
              {mode !== "forgot" && (
                <>
                  {/* Why the user was brought here — amber reads against the red page + dark card */}
                  {pendingRoute && (
                    <div
                      role="status"
                      className="flex items-center gap-3 mb-6 pl-3 pr-4 py-3 rounded-xl overflow-hidden"
                      style={{
                        background: "rgba(251,191,36,0.10)",
                        border: "1px solid rgba(251,191,36,0.35)",
                        borderLeft: "3px solid #fbbf24",
                        animation:
                          "authNoticeIn 380ms cubic-bezier(0.16,1,0.3,1) both",
                      }}
                    >
                      <svg
